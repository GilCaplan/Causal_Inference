...
Prenatal smoking and adverse infant outcomes are among the most
...
angiogenesis, all converging on hypoxia, growth restriction, preterm
delivery, and placental abruption~\cite{cnattingius2004}.
Wisborg et al.~\cite{wisborg2000} found a dose-dependent
...
adjustment for socio-demographic variables. Yuan et al.\ however adjust
...
In short: the existing literature consistently shows a \emph{positive association}
between prenatal smoking and infant mortality, with well-established biological
plausibility. The prior methods fall short of a causal standard because they
(a) do not state a potential-outcomes estimand, (b) condition on
post-treatment mediators, (c) do not assess overlap or positivity, and
...
The source is \emph{administrative}, not a survey: the outcome
(death linkage) and most confounders (age, education, insurance, parity,
pre-existing conditions) come from official records. Smoking items,
however, are self-reported by the mother at delivery; the misclassification
...
The primary unobserved confounders not captured in the data are alcohol and
substance use, together with maternal mental health. Birth weight and gestational
age lie on the causal path (mediators) and are excluded from adjustment
so that we estimate the total effect.

The DAG encodes several key relationships worth making explicit.
Each observed confounder has arrows pointing to both $T$ (treatment) and $Y$
(outcome), representing the backdoor paths we need to close. Unobserved
confounders (dashed) also point to both nodes, which is precisely why
sensitivity analyses are necessary: we cannot block those paths through
conditioning. The mediators ($\text{birth weight} \to Y$ and
$\text{gestational age} \to Y$) receive arrows from $T$ as well, illustrating
why including them in a regression would partially block the very effect
...
fetal hypoxia) are not version-specific and operate regardless of
cigarette brand or delivery method. That said, mothers do differ in how
many cigarettes they smoke per day, which means our binary treatment
averages over varying intensities. Section~\ref{sec:sensitivity} includes
a dose--response sensitivity check specifically to address this: if the
binary treatment were distorting the estimate, we would expect the effect
to be non-monotone across dose thresholds. It is not.
...
and age). The main unobserved threats, alcohol use and maternal
...
We use multiple estimators, all targeting the ATE; the ATT
...
and the difference averaged. This provides a stable baseline but may
shrink the treatment coefficient.

\textbf{T-Learner.} Separate logistic models for $T=1$ and $T=0$;
potential outcomes are predicted for all units and averaged. This allows
...
difference in weighted outcome means. This estimator is sensitive to extreme weights,
...
Figures~\ref{fig:ps_hist} and~\ref{fig:ps_logit} show the overlap between
treatment and control propensity score distributions, on the probability
and logit scales respectively.

The treated distribution (blue) is centred near PS\,$\approx$\,0.21 and
the control distribution (orange) near PS\,$\approx$\,0.04. There is overlap
across the common-support range (up to PS\,$\approx$\,0.85), but it is thin at
higher PS values: only 3.4\% of treated units fall below the median PS of
controls. The logit-scale density plot (Figure~\ref{fig:ps_logit}) makes the
separation more visible: the treated mass is centred around logit-PS\,$\approx -1.3$
whereas the controls spread well into the left tail (logit-PS\,$< -4$).
The pronounced mass of controls near PS\,$\approx$\,0 reflects the practical
positivity violation identified above.
...
diagonal closely across the predicted-probability range, so the
...
Two choices follow from this. First, AIPW is the primary estimator,
since it augments IPW with an outcome model that corrects for residual
imbalance. Second, trimming is the approach for preferred inference, since extreme
...
\textbf{Interpretation.} All five adjusted estimators agree on direction:
...
alcohol range), and heavy prenatal alcohol use is rare (roughly 1\%
...
effect on mortality that is not supported by the literature for either
confounder.
...
Because infant death is rare (roughly $4.5/1{,}000$), one within-group SD
of the outcome ($\sigma \approx 0.075$ in probability units) is large
...
(prevalence difference below 10\,pp between arms, mortality effect
below $5/1{,}000$) produces a shift well below these thresholds for
...
5th/95th trim, restricting to the region of best overlap, gives
...
dose--response pattern strengthens the causal interpretation. It is hard for
a confounder to produce a monotone gradient that mirrors
...
to the treated group. The stability across definitions confirms the
result is not sensitive to the precise operationalisation of
...
significant} estimate: ATE\,=\,$+1.73/1{,}000$ (95\% CI $[+0.05,\;+3.42]$). Multiple
births do not inflate the full-sample estimate; if anything,
...
is not an artefact of the matching configuration.
...
(4) stability across treatment operationalisations and matching
configurations; (5) a validated propensity model (calibrated, passes
the placebo permutation test); and (6) sensitivity analyses showing
...
Our results corroborate the existing literature while placing
